namespace SRMatrixNetwork.Commands
{
    ...
    public abstract class CommandBase
    {
        ...
        protected virtual CommandResult ParseArguments(string command)
        {
            try
            {
                ...
                if (split.Length < MandatoryParamCount + 1)
                    return CreateError(
                        $"Wrong arguments count. Got {split.Length - 1}, expected {MandatoryParamCount}.\nCommand syntax: {CommandHelpString}.");
        ...
        protected CommandResult EnsureState(CommandState state)
        {
            ...
            {
                result = CreateError(
                    $"Incorrect command state. Current state is '{Enum.GetName(typeof(CommandState), State)}',  expected state is '{Enum.GetName(typeof(CommandState), state)}'. Command terminated.");
            }
